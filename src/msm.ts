  ...
  execute: async (input) => {
    ...
    if (input.action === 'ccc-config') {
      return [
        ...
        'Unlike loop (task-driven, one-shot), resident runs forever via a double while loop:',
        '  outer = process lives forever; inner = one lifetime (lifetimeMs),',
        '  ending in self-termination -> new headless session -> new lifetime.',
        '',
        'Config lives in a SEPARATE file (not .opencode/serenity.json):',
        '  <CCC-root>/.serenity-meta/resident.json',
        'with an accompanying mind file (required, must exist & be parseable):',
        '  <CCC-root>/.serenity-meta/mind.md',
        '',
        ...
        '    description  — identity description, injected into template',
        ...
        '',
        '  Run commands (via resident tool):',
        '    resident start    — start the resident (reject if already running)',
        '    resident status   — query state (running/stale/stopped) with PID liveness check',
        '    resident stop     — SIGTERM, best-effort mind solidify then exit',
        '',
        '  Notes:',
        '    - mind.md is the resident\'s ONLY persistent memory; it is atomically',
        '      rewritten every round (tmp + rename). Agent may die — the mind on disk',
        '      is its recovery source.',
        '    - Recovery after process death = manual "resident start" (M0: no self-bootstrap).',
        '    - Fixed port derived from name (31000-61000); port is NOT in activePorts',
        '      so plugin dispose does not kill the resident.',
        '    - .serenity-meta/ should be gitignored (runtime state, not source).',
      ].join('\n');
    }
