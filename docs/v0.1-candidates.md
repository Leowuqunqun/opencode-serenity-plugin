# v0.1 Candidates — Reference Plugins Code Analysis
...
| 改进 | 复杂度 | 价值 | 依赖 |
|------|:---:|------|------|
| **Pre-indexed resources 完整版** | ~200 行 | 安全 ↑↑ | 需要 msm 设计变更 |
| **Hashline Edit 集成** | ~100 行 | LLM edit 成功率 ↑↑ | `@oh-my-opencode/hashline-core` npm 包（已开源）|

## v0.1 实施总览（待用户拍板）

| 项 | 新增 | 重构 | 测试 |
|---|:---:|:---:|:---:|
| 候选 1：两阶段 init | ~30 行 | — | ~3 tests |
| 候选 2：Pre-indexed 轻量 | ~80 行 | — | ~4 tests |
| 候选 3：Hook 工厂分层 | — | ~100 行 | 现有 24 tests 需重跑 |
| **合计** | ~110 行 | ~100 行 | 7+ tests |

**预计时间**：2-3 小时（含 typecheck + test + build 验证 + commit + push）
