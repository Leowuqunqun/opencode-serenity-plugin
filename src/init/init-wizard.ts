...
function generatePhase2Prompt(prefix: string): string {
  ...
  return [
    ...
    `## Your Task`,
    '',
    `Interview the user with the following 5 questions, then write the completed root SKILL.md at \`.opencode/skills/${cccName}/SKILL.md\`.`,
    `If the user skipped any question, accept a default answer — do not press.`,
    '',
    `### Question 1 — Git remote`,
    `"This CCC has a git remote configured. Confirm or update?"`,
    `→ If not yet configured, ask: "Want to add a git remote? (e.g., git@github.com:user/repo.git)"`,
    `→ If user says "skip" or "no", update SKILL.md to note \`Git remote: (local only)\`.`,
    `→ If user provides a URL, run \`cc-git\` to set it up.`,
    '',
    `### Question 2 — What does this CCC manage?`,
    `"List what this CCC tracks — repos, projects, configs, devices, etc."`,
    `→ Fill the **Task Route Table** section. Examples to offer:`,
    `  "my blog repo", "home network config", "personal notes", "project docs"`,
    `→ If user says "not sure" or "nothing yet", fill with \`(add your projects here)\` placeholder.`,
    '',
    `### Question 3 — Formality level`,
    `"Casual or structured?"`,
    `→ Casual: quick commits, short notes, ad-hoc sessions`,
    `→ Structured: detailed commits, formal docs, naming conventions, session tracking`,
    `→ Fill the **Collaboration Protocols** section accordingly.`,
    '',
    ...
    `→ Default: "conversational" if user skips.`,
    '',
    `### Question 5 — Extra skills or services?`,
    `"Need any extra skills? Managing servers, APIs, or devices?"`,
    `→ If user says "not sure" or "no": skip this — they can add skills later.`,
    `→ If user names something specific: note it in **Related Skills** section.`,
    `→ Pre-installed skills (compass, session, sqc) are already set up.`,
    '',
    '## After the Interview',
    '',
