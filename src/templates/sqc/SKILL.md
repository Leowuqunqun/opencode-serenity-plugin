...
| 维度 | 检查项 | 检查方式 |
|------|--------|---------|
| **DC-1** | 所有技能目录是否存在对应的 SKILL.md | 手动 / sqc-tool |
| **DC-2** | SKILL.md 中是否包含 name 和 description | 手动 / sqc-tool |
| **DC-3** | Skill frontmatter 中的 name 与技能目录名是否一致 | 手动 / sqc-tool |
| **DC-4** | {{ccc_name}} SKILL.md 的"相关技能"表是否包含所有已安装技能 | 手动 / sqc-tool |
...
| **GP-4** | SKILL.md 是否符合模板格式要求 | sqc-tool |

## MSM 工具

| MSM | 类型 | 用途 |
|-----|------|------|
| `sqc-tool` | semi-mech | SQC 5 阶段流水线编排 + 确定性检查 + 报告生成 |
...
2. 运行全量 SQC 流水线：
   ...
   ```
3. 检查报告中的警告和错误
4. 修复可自动化的问题（目录缺失、链接断裂、测试缺失）
5. 对需要人工判断的问题打开 AGENT_SESSION
