  ...
  execute: async (input) => {
    ...
    if (input.action === 'ccc-config') {
      return [
        ...
        '  ════════════════════════════════════════════',
        '',
        '  The resident is the CCC\'s single top-level persistent agent (v0.8 M0).',
        '  It answers a gap in the ACC: loop is task-driven and one-shot (run a task',
        '  to completion, then exit); nothing in ACC sustains a continuously-operating',
        '  cognitive subject. The resident fills that gap.',
        '',
        '  Core principle: CCE manages cognitive CONTINUITY; the resident manages',
        '  cognitive ACTIVITY. It is a "resident" of the CCC — has identity, state,',
        '  and a lifecycle, and answers to the final user.',
        '',
        '  Key design decisions (each with rationale):',
        '',
        '  A. Double while loop (永存 + 生命周期)',
        '     outer while = process lives forever (never self-exits)',
        '     inner while = one lifetime (lifetimeMs), then self-terminates',
        '       -> writes mind -> new headless session -> new lifetime',
        '     Why: context grows without bound in a live session. Instead of tracking',
        '     tokens, the resident resets context on a TIME boundary — simpler, no',
        '     token-accounting dependency. Each lifetime = one fresh session; the',
        '     mind.md carries continuity across lifetimes.',
        '',
        '  B. mind.md = the ONLY persistent memory (心智即身份)',
        '     The mind file is atomically rewritten every round (tmp + rename).',
        '     The agent may die at any moment — the mind on disk is its sole basis',
        '     for recovery. "Agent can die, but writes the mind before dying."',
        '     The mind must be self-contained: an agent that has never met the',
        '     resident must be able to continue its cognition from the mind alone.',
        '',
        '  C. Time boundary, not token boundary (时间界限)',
        '     lifetimeMs sets how long one lifetime lasts. The resident self-terminates',
        '     when remaining time runs out. Each round\'s POST timeout is capped at',
        '     min(timeoutMs, remaining + grace) so the time boundary is enforceable',
        '     even during a slow round. No token statistics needed.',
        '',
        '  D. Purpose-free by design (目的性不能重)',
        '     The config deliberately has NO "task" field. The resident is a general',
        '     cognitive subject, not bound to one domain (SQC/subtitles/xRP are all',
        '     things it may schedule, not its identity). Tasks come dynamically from',
        '     the mind.md task queue, decided by the resident itself.',
        '',
        '  E. Constraints inherited, not redefined',
        '     The resident has NO separate constraints section — it obeys the same',
        '     ACC constraints as any agent (path guards, MSM channel, permissions).',
        '     Security is delegated to the mind (the resident itself must encode',
        ...
        '  ════════════════════════════════════════════',
        '',
        '  Step 0 — Create the config files:',
        '',
        '  Config: <CCC-root>/.serenity-meta/resident.json',
        '  Mind:   <CCC-root>/.serenity-meta/mind.md  (required, must exist & be parseable)',
        '',
        ...
        '    "mind": {',
        '      "file": ".serenity-meta/mind.md"',
        '    },',
        '    "cycle": {',
        '      "type": "forever",',
        '      "intervalMs": 3600000,   // wake every 1h',
        '      "timeoutMs": 7200000,    // per-round timeout 2h',
        '      "lifetimeMs": 21600000   // one lifetime = 6h',
        '    }',
        '  }',
        '',
        '  Example mind.md (suggested structure, the resident itself evolves it):',
        '  # mind: guardian',
        '  ',
        '  ## identity',
        '  ## current goals',
        ...
        '',
        '  Fields:',
        '    name         — resident name (CCC unique)',
        '    description  — identity description, injected into the round template',
        '    model        — provider/model (must be installed, validated on start)',
        '    mind.file    — mind.md path relative to CCC root (must exist on start)',
        '    cycle.type       — "forever" (only supported type)',
        '    cycle.intervalMs — wake interval between rounds',
        '    cycle.timeoutMs  — per-round response timeout; must be >= intervalMs',
        '    cycle.lifetimeMs — one lifetime duration; must be > intervalMs',
        '',
        '  Step 1 — Start:',
        '    resident start',
        '    Returns: {ok, started, pid, port, status, log}',
        '    - status "initializing" = spawned, model validating / server booting',
        '    - status "running" = fully up',
        '    - log = path to the runner log file (/tmp/serenity-bg-task/resident-<port>.log)',
        '    - If already running: {ok:false, reason:"already_running"}',
        '',
        '  Step 2 — Check state:',
        '    resident status',
        '    Returns: {running, status, pid, port, lifetimeCount, roundInLifetime,',
        '              remainingMs, lastMindWrite, lastHeartbeat, ...}',
        '    status values:',
        '      running      — process alive & is the resident',
        '      initializing — spawned, booting',
        '      recovering   — serve crashed, rebuilding',
        '      stale        — status file exists but process dead (or PID reused)',
        '      unknown      — no status file yet',
        '',
        '  Step 3 — Stop:',
        '    resident stop',
        '    Sends SIGTERM; runner best-effort solidifies the mind (writes the last',
        '    extracted mind to disk) then cleans up serve and exits.',
        '    NOTE: mind solidify is "most likely" not "guaranteed" — an in-flight',
        '    round\'s mind may be lost (at most ~1 interval of staleness).',
        '',
        '  Recovery after process death (machine reboot, crash):',
        '    Simply run "resident start" again. The mind.md on disk is the recovery',
        '    source — the resident resumes from where it last wrote its mind.',
        '    M0 has NO self-bootstrap (no systemd/cron). Human starts it.',
        '',
        '  What the resident does each round:',
        '    1. Reads its mind (injected into the round template)',
        '    2. Advances one cycle of work (its own goals / task queue)',
        '    3. Outputs a COMPLETE mind snapshot in ---MIND-BEGIN---/---MIND-END---',
        '    4. Runner atomically writes that snapshot back to mind.md',
        '    5. Sleeps intervalMs; repeats until lifetime runs out, then self-terminates',
        '       and starts a fresh lifetime (new session, mind carries on)',
        '',
        '  Early termination (STOP signal) — the resident may end a lifetime early when:',
        '    - remaining time insufficient for a full new round,',
        '    - the mind needs a wholesale reset (direction wrong),',
        '    - work has converged.',
        '',
        '  Recommended first use: SQC quality cycle',
        '    Give the resident a mind whose task queue includes periodic SQC scans',
        '    (msm_admin check + ontology compliance). It will wake, scan, fix what is',
        '    automatable, and update its mind — no human in the loop.',
        '',
        '',
        '  ════════════════════════════════════════════',
        ...
        '  ════════════════════════════════════════════',
        '',
        '  - .serenity-meta/ must be gitignored (runtime state, not source):',
        '    add ".serenity-meta/" to <CCC-root>/.gitignore',
        '  - Fixed port derived from CCC name + resident name (31000-61000);',
        '    NOT registered in activePorts, so plugin dispose does not kill it.',
        '  - Runner is spawned detached; killing the host opencode session does NOT',
        '    stop it (it is a true daemon). Use "resident stop" to stop.',
        '  - Logs: runner -> /tmp/serenity-bg-task/resident-<port>.log;',
        '    serve -> /tmp/serenity-bg-task/server-<port>.log.',
        '  - The resident uses the same ACC permission model as any agent:',
        '    it can run tools (msm_exec, cc-fs, cc-git...) and write within the CCC.',
        '    There is NO extra sandbox in M0 — the mind must encode its own discipline.',
        '  - If "resident start" returns ok:false/unconfirmed, read the log file',
        '    in the returned "log" field.',
      ].join('\n');
    }
